...
pub struct AppSettings {
    ...
    /// WhisperFly cloud mode: ship each finished voice note to a FlyMyAI agent
    /// (cloud STT + cleanup + keywords + Notion filing). Off by default; the
    /// local transcription flow is never blocked by it.
    #[serde(default)]
    pub whisperfly_cloud_enabled: bool,
    /// FlyMyAI API key (env FLYMYAI_API_KEY works as a fallback).
    #[serde(default)]
